<div align="center">

<!-- Premium Header -->
<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0F2027,50:203A43,100:2C5364&height=260&section=header&text=Anglo%20Saber&fontSize=65&fontColor=ffffff&animation=fadeIn&fontAlignY=35&desc=Generative%20AI%20Engineer%20%7C%20NLP%20Researcher%20%7C%20Senior%20iOS%20Developer&descAlignY=65" width="100%"/>


<!-- Typing Animation -->
<img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=700&size=24&pause=1000&color=58A6FF&center=true&vCenter=true&width=950&height=50&lines=🤖+Building+Intelligent+AI+Applications;🧠+Exploring+LLMs+%7C+RAG+%7C+AI+Agents;🔎+Applied+NLP+Research+%7C+Generative+AI;📱+SwiftUI+%7C+iOS+Engineering+%7C+AI+Integration"/>


<br>


<img src="https://komarev.com/ghpvc/?username=anglosaberdev&label=Profile%20Views&color=58A6FF&style=for-the-badge"/>

</div>


---

# 👨‍💻 About Me

Hi, I'm **Anglo Saber** 👋

🚀 **Generative AI Engineer | NLP Researcher | Senior iOS Developer**

I specialize in designing and building intelligent applications that combine:

- 🤖 Large Language Models (LLMs)
- 🔎 Retrieval-Augmented Generation (RAG)
- 🧠 AI Agents & Agentic Systems
- 🔤 Transformer-based NLP Applications
- 🗄️ Vector Databases & Semantic Search
- 📱 AI-powered Mobile Applications


My mission is to transform advanced AI research into scalable real-world products.


---

# 🎯 Core Focus Areas


<p align="center">

<img src="https://img.shields.io/badge/Generative_AI-LLMs-blue?style=for-the-badge&logo=openai"/>

<img src="https://img.shields.io/badge/RAG-Vector_Search-purple?style=for-the-badge"/>

<img src="https://img.shields.io/badge/NLP-Transformers-green?style=for-the-badge"/>

<img src="https://img.shields.io/badge/iOS-SwiftUI-orange?style=for-the-badge&logo=swift"/>

</p>


---

# 🛠️ Technical Stack


<table width="100%">

<tr>

<td width="50%" valign="top">

## 💻 Languages & Backend


<img src="https://skillicons.dev/icons?i=python,swift,java,javascript,typescript,fastapi,flask,docker&perline=4"/>


### Languages

- Python
- Swift
- Java
- JavaScript
- TypeScript


### Frameworks

- FastAPI
- Flask
- SwiftUI
- UIKit
- Combine


</td>


<td width="50%" valign="top">


## 🧠 AI & Machine Learning


<img src="https://skillicons.dev/icons?i=pytorch,tensorflow&perline=2"/>


### Deep Learning

- PyTorch
- TensorFlow
- Scikit-learn


### Generative AI

- LangChain
- LlamaIndex
- Hugging Face Transformers
- IBM watsonx.ai
- OpenAI API


</td>

</tr>

</table>


---

# 🔎 Retrieval & Vector Intelligence


- ChromaDB
- FAISS
- Vector Databases
- Embedding Models
- Semantic Search
- Similarity Search
- RAG Architectures


---

# 🏆 Certifications & Professional Learning


## 🤖 Generative AI Engineering & LLMs


| Certification | Provider |
|---|---|
| Generative AI Engineering with LLMs Specialization | IBM - Coursera |
| Generative AI Engineering and Fine-Tuning Transformers | IBM - Coursera |
| Generative AI Language Modeling with Transformers | IBM - Coursera |
| Gen AI Foundational Models for NLP & Language Understanding | IBM - Coursera |
| Generative AI and LLMs: Architecture and Data Preparation | IBM - Coursera |


---

## 🔎 RAG & AI Agents


| Certification | Provider |
|---|---|
| Fundamentals of AI Agents Using RAG and LangChain | IBM - Coursera |
| Project: Generative AI Applications with RAG and LangChain | IBM - Coursera |
| Build RAG Applications: Get Started | IBM - Coursera |
| Vector Databases for RAG: An Introduction | IBM - Coursera |


---

## 🧠 NLP


| Certification | Provider |
|---|---|
| Natural Language Processing Specialization | DeepLearning.AI - Coursera |
| Natural Language Processing in Python | Udemy |
| Introduction to Transformer Models for NLP | Pearson |


---

## 📱 iOS Engineering


| Certification | Provider |
|---|---|
| MVVM Design Pattern Using Swift in iOS | Udemy |
| Testing iOS Apps - BDD Using Swift | Udemy |
| Visual Swift Memory Mastery | Udemy |


---

# 🚀 Featured Projects


## 🤖 Generative AI Chat Application

Flask-based AI chatbot application powered by Large Language Models.


**Technology Stack**

```

Python
Flask
LLMs
OpenAI API
Hugging Face

```


---

## 🧠 Generative AI Assistant with Gradio

Interactive AI assistant for LLM experimentation and prompt workflows.


**Technology Stack**

```

Python
Gradio
LangChain
Prompt Engineering

```


---

## 🔎 RAG Knowledge Assistant

Document intelligence system using Retrieval-Augmented Generation.


**Technology Stack**

```

LlamaIndex
ChromaDB
Embeddings
Vector Search
RAG

```


---

# 📊 GitHub Analytics


<div align="center">


<img height="190" src="https://github-readme-stats.vercel.app/api?username=anglosaberdev&show_icons=true&theme=transparent&hide_border=true&include_all_commits=true&count_private=true&title_color=58A6FF&icon_color=58A6FF&text_color=c9d1d9"/>


<img height="190" src="https://github-readme-stats.vercel.app/api/top-langs/?username=anglosaberdev&layout=compact&theme=transparent&hide_border=true&title_color=58A6FF&text_color=c9d1d9"/>


</div>


---

# 📈 Contribution Activity


<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=anglosaberdev&bg_color=0D1117&color=58A6FF&line=8B5CF6&point=FFFFFF&area=true&hide_border=true"/>

</div>


---

# 🤝 Connect With Me


<div align="center">


<a href="https://github.com/anglosaberdev">

<img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>

</a>


<a href="https://linkedin.com/in/anglosaber">

<img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>

</a>


<a href="mailto:eng.angloessam2010@gmail.com">

<img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>

</a>


</div>


---

# 💡 Philosophy


> "Building intelligent systems where AI meets human creativity." 🚀🤖


<div align="center">

⭐ **Always learning. Always building. Always improving.**

</div>


<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0F2027,50:203A43,100:2C5364&height=120&section=footer"/>

</div>
